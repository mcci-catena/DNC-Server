...
exports.readtags = (req, res) => {
    var filter = {"name": req.body.influxd.uname}
...
exports.getFields = (req, res) => {
    var filter = {"name": req.body.cname}
    Client.findOne(filter)
    .then(async function(data) {
            if(data)
            {
...
function getDNCTagValues(data1, tagname, filtdict)
{
    return new Promise(function(resolve, reject) {
        var tagval = [];
        spot = spotctrl.getschema(data1.id, data1.tags)
        spot.find(filtdict)
        .then(data => {
            if(data)
            {
